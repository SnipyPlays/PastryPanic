...
`DynamicScene`, to the `com.github.hanyaeger.pastrypanic.scenes`
package. Use the method `setupScene()` to set the background to the
asset `background2.jpg` and the audio to `pastryPanic.mp3`.
...
`com.github.hanyaeger.pastrypanic.entities.buttons`. Use the following constructor:
...
```java
private Waterworld pastryPanic;

public TitleScene(Waterworld pastryPanic){
    this.pastryPanic = pastryPanic;
}
```
